...
ogImage:
    url: "/img/posts/en/how-i-made-my-blog/cover.jpg"
    width: 1320
    height: 1200
---
...
I had a lot of choices to make for features to either snuff out or include. I got started on the design while simultaneously thinking about what tech I would use, and what was viable or not given the scope of the project.

Originally I had a totally different idea to how the site was going to look. I would have a vectorised Japanese ukiyo-e painting serving as the key hero image, and animate all the different features of the painting would. The foreground and background would shift with a parallax effect, clouds and boats would drift by, smoke would rise out from a village in the background etc.
...
<figure>
    ...
    />
    <figcaption>
        My orignal site design feat vectorised image of one of Hiroshige's
        paintings of Hakone. Note the old color scheme for Vincent Van Gogh
    </figcaption>
</figure>

While it was neat idea, and the vector image was already prepared, I knew it would take an age to code, and it might not necessarily turn out the way I wanted.
...
I used a Adobe XD for laying out the site and prototyping, and built the assets (icons, Vincent Van Gogh portrait etc.) in Adobe Illustrator.
...
**_But_** seeing as before learning React I was a Wordpress dev, I had to weigh up whether using NextJS over Wordpress was a good choice for me and the site.

Switching to creating a blog using NextJS involved re-learning a lot of concepts when it comes to blog creation. For example, I had to build **_everything_** from scratch. Wordpress hands all the basic blog features to you on a silver platter. Categories, tags, thumbnails and querying for getting articles. Accessing post meta-data and adding custom fields and so on. All of this functionality had to implemented _manually_ in a NextJS blog.

However, at the risk things being implemented incorrectly, it _does_ allow for much greater freedom.
It also just feels a lot more _scaleable_ using React's component/prop way of doing things. Creating a component-based site in Wordpress just feels kind of _wrong_ once you get used to doing it in React. Templating techniques such as using `get_template_parts` with `$args` just feels second class in Wordpress compared to how it's managed in React.

What's also huge is when you compare the end site's user experience.
Having the whole NextJS site as a single application with page transitions feels so much better than an Apache Wordpress site every could.
...
I'll definitely be using NextJS in future, more complex projects (one's where using NextJS is warranted), so I thought I might as well dive straight into NextJS from the very beginning of my React career.
...
I needed something simple, and initially looked into using _Markdown_ files (MD for short – a way of writing and storing mixed text/media content) to write and store posts. I am the only one involved in creating and updating blog posts, and they were'nt going to be particularly media-rich posts, so I thought MD files would do just fine.
...
I stumbled upon a thing called MDX (Markdown with JSX support – allowing articles to have embedded React components) through Josh Comeau's [blog post](https://www.joshwcomeau.com/blog/how-i-built-my-blog/), I knew I had to incorporate this into my site. The primary thing I'd be able to use MDX for was to to include code snippets with _syntax highlighting_ into my blog posts.

Once I chose to switch from vanilla Markdown to MDX, I had to update my file parser to support the new `.mdx` files.
There were a few options for parsers, but I ended up opting for [mdx-bundler](https://github.com/kentcdodds/mdx-bundler). It allows you to parse the MDX file inside of `getStaticProps`, and then pass the parsed content to the React component inside your page where it is rendered.

The parser also allows you to overwrite the default HTML spat out by parsing process, so you can specify the replacing of certain elements with your own custom components.
I do this with my HTML headings, where the conversion of my titles into `<h1>` and `<h2>` tags are automatically replaced with my own custom heading components, which display anchor links beside the title text (in my code, this magic happens [here](https://github.com/veritasnz/seanv-site/blob/main/components/Posts/PostBody.js)).
...
If you click on the '日本語' button in header menu, the language will change to Japanese. The multilingual aspect of this site was surprisingly one of the easier features to implement.
...
Through this library you set and create your namespaces _JSON_ files and the corresponding pages that they should be loaded by, and then through their custom `useTranslation()` hook you can retrieve the translations per namespace as needed. The placement of files and details of how to set up the `i18n.json` settings file can all be found in their [documentation](https://github.com/vinissimus/next-translate#add-i18njs-config-file).
...
I thought this would be a simple addition, however it created quite the headache. After searching all over for something that worked, I found a solution (shout out to [this article by James Wallis](https://wallis.dev/blog/nextjs-page-transitions-with-framer-motion)) using Framer Motion.
...
I'm guessing the NextJS build step over-optimised the SASS Modules that I had been using for styling my app up until then. It must have been discardeding the previous page's styles at the start of the transition... Either way apparently it is a known (and unfixed) bug.
...
I spent 90% of my time coding this app with SASS modules. But this wouldn't fly with the Framer Motion page transition/NextJS bug.

With a single global stylesheet, I lost some performance benefits from lazy-loading styles, classname minification, and also the safetly of locally-scoped styles.

Completely refactoring my app to work with a single global SASS stylesheet was painful. But, it's a tiny site, and I was already efficent at coding safe and maintainable stylesheets, so sacrificing the drawbacks for slick page transitions ultimately felt worth it.
...
-   RSS feed
-   Code some dummy one-off 'sandbox' pages that show design concepts and artstyles
-   Ability to filter works by type (personal vs client)
-   Make the Hero Vincent Van Gogh's eyes follow the users cursor on hover
-   Get CSS modules working with MDX (hard)

Actually probably should get around to :
...
-   Check amount of people visiting in Safari and provide smooth-scroll polyfill
...
-   Email objuscation – [react-obfuscate](https://github.com/coston/react-obfuscate)
